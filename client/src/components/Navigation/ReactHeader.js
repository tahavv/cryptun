import React, { Component } from 'react';
import { Link } from 'react-router-dom';
import { Icon,Layout, Menu,Button } from 'antd';
import ContactsOutlined from '@ant-design/icons/ContactsOutlined';
import { iconStyle } from '../../styles';
import logoImage from '../../images/logo/logo.png';
import { connect } from 'react-redux';
import { setHeaderMenuItem } from '../../redux_actions'; 



const { Header } = Layout;

class ReactHeader extends Component {

    render() {
        const { selected } = this.props;
        return (
            <React.Fragment>
               <Header className="header">
                    <Menu
                        theme="dark"
                        mode="horizontal"
                        selectedKeys={[selected]}
                        style={{ lineHeight: '64px'}}
                    >
                        <Menu.Item key="logo" onClick={()=>this.props.setHeaderMenuItem('home')}>
                            <Link to="/">
                            <img alt="crypto react app logo"
                                 style={{ maxHeight: '60px',maxWidth: '180px'}}
                                 src={logoImage}></img>
                            </Link>
                        </Menu.Item>
                        <Menu.Item key="home" onClick={()=>this.props.setHeaderMenuItem('home')}>
                            <Link to="/">
                                <Icon type="home" style={iconStyle}/>Home
                            </Link>
                        </Menu.Item>
                        <Menu.Item key="dashboard" onClick={()=>this.props.setHeaderMenuItem('dashboard')}>
                            <Link to="/status/list">
                                <Icon type="area-chart" style={iconStyle}/>Dashboard
                            </Link>
                        </Menu.Item>
                        <Menu.Item key="about" onClick={()=>this.props.setHeaderMenuItem('about')}>
                            <Link to="/about">
                                <Icon type="info-circle" style={iconStyle}/>About
                            </Link>
                        </Menu.Item>
                        <Menu.Item key="contact" onClick={()=>this.props.setHeaderMenuItem('contact')}>
                            <Link to="/contact">
                            <ContactsOutlined style={iconStyle}/>Contact
                            </Link>
                        </Menu.Item>
                        <Menu.Item key="login" onClick={()=>this.props.setHeaderMenuItem('login')}>
                            <Link to="/login">
                                    <Button type="primary" style={{ width: 100 ,minHeight:50 ,marginLeft:300}}>LOGIN </Button>
                            </Link>
                        </Menu.Item>
                        <Menu.Item key="register" onClick={()=>this.props.setHeaderMenuItem('register')}>
                            <Link to="/register">
                                    <Button type="primary" ghost style={{ width: 100 ,minHeight:50 }}>Get Started </Button>
                            </Link>
                        </Menu.Item>
                    </Menu>
                </Header>
            </React.Fragment>
        )
    }
}

const mapStateToProps = (state) => {
    return {
        selected: state.header_selected
    }
}

const mapActionsToProps = {
    setHeaderMenuItem
}

export default connect(mapStateToProps, mapActionsToProps)(ReactHeader);
